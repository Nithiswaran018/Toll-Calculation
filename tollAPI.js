...
const App = () => {
  ...
  useEffect(() => {
    ...
    const polyline = L.polyline([startCoordinate, endCoordinate]).toGeoJSON().geometry.coordinates;
    const encodedPolyline = encodeURIComponent(polyline.map(point => point.reverse()).join(';'));

    // TollGuru API endpoint
    const apiUrl = `https://dev.tollguru.com/v1/calc/route?from=${startCoordinate[0]},${startCoordinate[1]}&to=${endCoordinate[0]},${endCoordinate[1]}&vehicleType=2AxlesAuto&departure_time=2023-01-01T12%3A00%3A00Z&polyline=${encodedPolyline}`;
